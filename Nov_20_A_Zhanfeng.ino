...
// sensorReadingNum = fr * 8 + fl * 4 + rr * 2 + rl
int readLineSensor(){
...
void goStraight(int stop_condition){
  while(1){
    ...
    if(stop_condition == LR && rr == 1 && rl == 1){return;}
    if(stop_condition == L && rl == 1){return;}
    if(stop_condition == R && rr == 1){return;}
...
bool followLine(int stop_condition){
  while(1){
    ...
    else if(fr == 1){slightRight();}
    else if(fl == 1){slightLeft();}
    else{goFront();}
  ...
  if(blockProcessed == 0){
    switch(order){
      case 0:
        ...
        followLine(LR);
        stepForward();
        goStraight(LR); // back to the lower edge of the starting point
      case 1:
        turnLeft90Deg();
        stepForward();
        return followLine(LR); // back to the cross
      case 2:
        turnLeft90Deg();
        stepForward();
        return followLine(L); // back to the first corner
      case 3:
        turnLeft90Deg();
        stepForward();
        return followLine(LR); // back to the second corner
      case 4:
        turnLeft90Deg();
        stepForward();
        return followLine(L); // bsck to the third corner
      case 5:
        stepForward();
        return followLine(LR); // back to the fourth corner
    }
  }
  return;
}

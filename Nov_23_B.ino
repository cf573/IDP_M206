...
int LEFT_TURNING_INTERVAL = 1500; // the time motor takes to turn to 90 degrees in max speed
int RIGHT_TURNING_INTERVAL = 1350;
int TURNING_180_INTERVAL = 3150; // the time motor takes to turn to 90 degrees in max speed
int STEP_FORWARD_INTERVAL = 400;
...
void task1(){
  ...
  while(blockProcessed <= 1){
    ...
    if(isMagnetic()){
      ...
      delay(6000);
      changeLED(OFF);
      gotoRed(magneticBlockProcessed);
    }
    else{
      changeLED(GREEN);
      delay(6000);
      changeLED(OFF);
      gotoGreen(nonMagBlockProcessed);
    }
    blockProcessed++;
